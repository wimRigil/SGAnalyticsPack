...
/// The SGAnalytics Tracker is a Swift framework to send analytics to the SGAnalytics server.
///
/// ## Basic Usage
/// * Use the track methods to track your views, events and more.
final public class SGAnalytics: NSObject {
    ...
    @objc public var isOptedOut: Bool {
        get {
            return analyticsUserDefaults.optOut
        }
        set {
            analyticsUserDefaults.optOut = newValue
        }
    }
    ...
    public var visitorId: String? {
        get {
            return analyticsUserDefaults.visitorUserId
        }
        set {
            analyticsUserDefaults.visitorUserId = newValue
            visitor = Visitor.current(in: analyticsUserDefaults)
        }
    }
    
    internal var analyticsUserDefaults: SGAnalyticsUserDefaults
    ...
    /// This logger is used to perform logging of all sorts of SGAnalytics related information.
    ...
    public static var _sharedInstance: SGAnalytics?
    ...
    required public init(siteId: String, queue: Queue, dispatcher: Dispatcher) {
        ...
        self.analyticsUserDefaults = SGAnalyticsUserDefaults(suiteName: "\(siteId)\(dispatcher.baseURL.absoluteString)")
        self.visitor = Visitor.current(in: analyticsUserDefaults)
        self.session = Session.current(in: analyticsUserDefaults)
        super.init()
        startNewSession()
        startDispatchTimer()
    }
    ...
    @objc public func dispatch() {
        guard !isDispatching else {
            logger.verbose("SGAnalytics is already dispatching.")
            return
        }
...
extension SGAnalytics {
    ...
    public func startNewSession() {
        analyticsUserDefaults.previousVisit = analyticsUserDefaults.currentVisit
        analyticsUserDefaults.currentVisit = Date()
        analyticsUserDefaults.totalNumberOfVisits += 1
        self.session = Session.current(in: analyticsUserDefaults)
    }
}
...
extension SGAnalytics {
    ...
    /// Tracks a screenview.
    ///
    /// This method can be used to track hierarchical screen names, e.g. screen/settings/register. Use this to create a hierarchical and logical grouping of screen views in the SGAnalytics web interface.
    ///
...
extension SGAnalytics {
    ...
    /// - Parameter index: The index of the dimension. A dimension with this index must be setup in the SGAnalytics backend.
...
extension SGAnalytics {
    public func copyFromOldSharedInstance() {
        analyticsUserDefaults.copy(from: UserDefaults.standard)
    }
}
